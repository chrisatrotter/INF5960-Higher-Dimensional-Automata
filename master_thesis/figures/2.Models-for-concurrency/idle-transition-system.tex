...
\begin{document}
  ...
  \tikzstyle{every node}=[font=\footnotesize]
    ...
    \title{A idle transition system}
    \tikzset{every loop/.style={min distance=10mm,looseness=10}}

    \node[state](q1) [label=left:{$s_{1}$}]                        {};
    \node[state](q2) [above right of=q1, label=below:{$s_{2}$}]    {};
    ...
    \path[->] (q1) edge[loop] node[above] {*} (q1);
    \path[->] (q2) edge[loop] node[above] {*} (q2);
    \path[->] (q3) edge[loop] node[above] {*} (q3);
    \path[->] (q4) edge[loop] node[above] {*} (q4);
